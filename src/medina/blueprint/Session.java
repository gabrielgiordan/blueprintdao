...
public final class Session extends SessionManager
{
	...
	private final class SessionTransaction implements Transaction
	{
		...
		@Override
		public void addSavePoint(int index) throws TransactionException
		{
			try
			{
				savepoints.put
				(
					index, 
					connection.setSavepoint(String.valueOf(index))
				);
			}
			catch (SQLException e)
			{
		...
		@Override
		public void releaseSavePoint(int index) throws TransactionException
		{
			try
			{
				connection.releaseSavepoint
				(
					savepoints.get(index)
				);
		...
		@Override
		public void rollback(int savepointIndex) throws BlueprintException
		{
			try
			{
				connection.rollback
				(
					savepoints.get(savepointIndex)
				);
				...
				System.err.println
				(
					"	Transaction was rolled back at" +
					" savepoint of index " + savepointIndex + "."
				);
			}
			catch(SQLException e)
			{
